<template>
    <view class="stu-show">
        <el-row justify="space-between">
            <!-- PC -->
            <el-col class="hidden-sm-and-down" v-showMeta="(i + 1) % 2 === 0 ? `animate__fadeInRight` : `animate__fadeInLeft`" @tap="goToDetail(stu)"
                :xs="24" :sm="{ span: 11, offset: 1 }" :md="{ span: 12, offset: 0 }" v-for="stu, i  in currentStuData"
                :class="(i + 1) % 2 === 1 ? 'el-col-border' : ''" :key="i">
                <StuCard :data="stu" />
            </el-col>
            <!-- 移动端 -->
            <el-col class="hidden-md-and-up" v-showMeta="(i + 1) % 2 === 0 ? `animate__fadeInRight` : `animate__fadeInLeft`" @tap="goToDetailForMobile(stu)"
                :xs="24" :sm="{ span: 11, offset: 1 }" :md="{ span: 12, offset: 0 }" v-for="stu, i  in currentStuData"
                :class="(i + 1) % 2 === 1 ? 'el-col-border' : ''" :key="i">
                <StuCard :data="stu" />
            </el-col>
...
// 移动端跳转
const goToDetailForMobile = (data) => {
...
const currentStuData = ref([
    ...
    },
    {
        ...
        imgList: ['https://alumni.sem.tsinghua.edu.cn/__local/6/B4/F8/B2570650DFB5ED68BA91A2F2097_CBCC4FFA_2ED5B.jpg?e=.jpg']
    }
])
